...
import { doc, getDoc, setDoc } from "firebase/firestore";
import { AppDispatch } from "../store"; // import your store type
import { setUser, startLoading } from "../store/slices/authSlice";

const provider = new GoogleAuthProvider();

export async function handleGoogleLogin(dispatch: AppDispatch) {
  try {
    ...
    dispatch(
      setUser({
        uid: user.uid,
        email: user.email || "",
        role: (userSnap.data()?.role as "admin" | "user") || "user",
      })
    );
